...
```text
app.py
主程序，包含 Streamlit 页面、账户系统、学习逻辑、状态保存和 UI 样式。
...
- 当前仍使用 JSON 保存账户和学习状态，适合测试和小规模使用。
- Streamlit Community Cloud 的本地文件保存不适合长期稳定保存大量用户数据。
- 账户系统是轻量版，不是正式商业级用户系统。
- 保持登录通过 remember token 实现，用户不能分享带 `remember` 参数的网址。
- 目前没有 SQLite、Supabase 或 PostgreSQL 数据库。
- 目前没有管理员后台。
- 目前没有正式权限系统。
- 目前没有支付系统。
- 目前没有错题本、智能复习或模拟考试。
