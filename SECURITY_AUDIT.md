...
当前已新增登录用户修改自己密码的最小功能。

安全边界：

- 只有已登录用户可以在侧栏看到“修改密码”入口。
- 修改密码前必须输入当前密码。
- 新密码和确认新密码必须一致。
- 新密码不能和当前密码相同。
- 新密码最少 8 个字符。
- 修改成功后会重新生成 `salt` 和 `password_hash`。
- 修改成功后会清除该用户所有 remember token，并退出当前登录状态，提示用户用新密码重新登录。
- 修改成功会写入 `app_events` 事件：`password_changed`。
- 日志中不记录明文密码、remember token 或密钥。
...
- 只有 `admin` 或 `super_admin` 能在侧栏看到“管理员后台”入口。
- 后台页面函数开头仍会调用 `require_admin()`，普通用户即使手动触发也会被拦截。
- 后台第一版只允许上传 CSV 到内存中检查和预览。
- 不会覆盖正式 `words.csv`。
- 不会保存上传文件。
- 不会新增词库后台写入能力。
- 每次管理员上传预览文件会写入 `data/admin_logs.jsonl`。
...
- 当前已有第一版管理员后台，但只支持上传 CSV 预览，不允许修改正式词库或系统配置。
- 当前主要风险集中在 remember token 暴露、JSON 文件存储稳定性、缺少登录防爆破、管理员后台尚未接入正式写入和发布流程。
...
- 当前已有第一版管理员后台，仅允许管理员上传 CSV 进行字段检查和预览。
- 当前已有 `file_uploader` 词库上传预览入口，但没有正式词库写入入口。
...
- 已有侧栏“管理员后台”入口，仅管理员可见。
- 未发现独立 `/admin` 页面。
- 已有最小化管理员角色字段。
- 已有管理员 CSV 上传预览入口，但不写入正式词库。
- 未发现系统配置修改入口。
- 未发现隐藏按钮但缺少后端权限校验的问题。
...
当前管理员后台第一版没有正式写入能力，并且页面入口和后台函数均进行权限检查。
